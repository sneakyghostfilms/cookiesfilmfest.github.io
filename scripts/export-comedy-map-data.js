...
const OUTPUT_FILE = join(__dirname, '../comedy-festival-map.csv');
...
async function loadLocations() {
  try {
    ...
    if (error.code === 'ENOENT') {
      console.error('❌ No locations file found. Run `npm run geocode:comedy` first.');
      process.exit(1);
    }
    throw error;
  }
...
async function main() {
  console.log('📊 Comedy Festival Map Data Export');
  ...
  const withCoords = comedyFestivals.filter(f => locations[f.name]);
  ...
  if (missing.length > 0) {
    ...
    console.log('\n⚠️  Run `npm run geocode:comedy` to fetch missing coordinates\n');
  }
  console.log('━'.repeat(50));

  if (withCoords.length === 0) {
    console.error('❌ No festivals with coordinates. Run `npm run geocode:comedy` first.');
    process.exit(1);
  }
  ...
  console.log(`✅ Exported ${withCoords.length} comedy festivals to: comedy-festival-map.csv`);
  ...
  console.log('   4. Upload comedy-festival-map.csv');
